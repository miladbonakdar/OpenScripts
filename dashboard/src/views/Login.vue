                  ...
                  <b-row>
                    <b-col cols="6">
                      <b-button
                        ...
                        @click="signIn"
                      >
                        <i v-if="signingIn" class="fa fa-circle-o-notch fa-spin"></i> Login
                      </b-button>
                    </b-col>
...
export default {
  ...
  created() {
    if (localStorage.getItem("token")) this.$router.replace("/dashboard");
  },
  methods: {
    eneterKey(e) {
      if (e.keyCode === 13) {
    ...
    signIn() {
      ...
      if (this.hasError == false) {
        ...
        this.$gate.auth
          ...
          .catch(err => this.$toasted.global.error(err.data.Message))
